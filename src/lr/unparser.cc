...
typedef std::pair<wast_iterator_t, wast_iterator_t> wi_pair_base;

struct wi_pair : public wi_pair_base {
	wi_pair(wast_iterator_t f, wast_iterator_t s) : wi_pair_base(f, s) {}
	friend bool operator==(const wi_pair& a, const wi_pair& b) {
		return a.first==b.first && a.second==b.second;
	}
};

struct wi_pair_stack : public std::vector<wi_pair> {
	bool contains(const wi_pair&wip) const {
		const_iterator i, j;
...
static int _buf_endswith(const char* str) {
...
int unproduce_rule(wast_iterator_t grammar, wast_iterator_t expr, wast_iterator_t ast) {
	...
	if(!strcmp(wi_op(expr), "TransientRule")) {
		if(wa_check_lefty(wi_node(expr))) {
			...
			/*printf("entering transient rule (%s).\n", wi_string(expr, 0));*/
			wi_down(expr);
			wi_next(expr);
...
int unproduce(wast_iterator_t grammar, wast_iterator_t expr, wast_iterator_t ast/*, int* next*/) {
	...
	if(!strcmp(wi_op(expr), "T")) {
		...
		_buf_append_newline();
		status = 1;
	} else if(!strcmp(wi_op(expr), "Indent")) {
		_buf_do_indent();
		status = 1;
	} else if(!strcmp(wi_op(expr), "Dedent")) {
		_buf_do_dedent();
		status = 1;
...
const char* tinyap_unparse(wast_t grammar, wast_t ast) {
	...
	_buf_init();
	ig = wig_goto_rule(igrammar, (char*)"_start");
	/*wa_dump(ast);*/
	/*tinyap_walk(ast, "prettyprint", NULL);*/
	/*printf("\n");*/
